...
def count_empty_seats(chart)
  ...
  chart.reduce(0) do |nil_count, row_index|
    nil_count += row_index.reduce(0) do |nil_seats, seat_index|
      nil_seats += 1 if seat_index == nil
      nil_seats
    end
    nil_count
  end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.each do |obj|
    return obj if obj[:is_my_favorite?] == true
  end
  nil
end